...
test "produces visible pixels" {
    ...
    render.renderNotification(&pixels, 96, 96, 1.0, .copied);
...
test "zero alpha produces zero alpha channel" {
    ...
    render.renderNotification(&pixels, 48, 48, 0.0, .copied);
    for (0..48) |y| {
        for (0..48) |x| {
...
test "different kinds produce different output" {
    ...
    render.renderNotification(&p1, 96, 96, 1.0, .copied);
    render.renderNotification(&p2, 96, 96, 1.0, .override_hint);
...
test "scaled output has more filled pixels" {
    ...
    var p2: [96 * 96 * 4]u8 = undefined;
    render.renderNotification(&p1, 48, 48, 1.0, .copied);
    render.renderNotification(&p2, 96, 96, 1.0, .copied);
...
test "half alpha produces lower alpha values" {
    ...
    render.renderNotification(&p_full, 96, 96, 1.0, .copied);
    render.renderNotification(&p_half, 96, 96, 0.5, .copied);
...
test "all scales render without crash" {
    ...
    inline for (.{ 48, 72, 96, 144, 192 }) |s| {
        render.renderNotification(&buf, s, s, 0.8, .copied);
        render.renderNotification(&buf, s, s, 0.8, .override_hint);
    }
}
